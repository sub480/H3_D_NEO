...
def load_detector(name: str):
    from .pack import DEFAULT_DETECTOR, resolve_detector_path

    key = str(name or DEFAULT_DETECTOR).replace("\\", "/").strip() or DEFAULT_DETECTOR
    if key in _DETECTOR_CACHE:
        return _DETECTOR_CACHE[key]
    path = resolve_detector_path(key)
    if path is None:
        raise FileNotFoundError(
            f"FaceRefine 检测器 '{name}' 未找到。请将 face_yolov8m.pt 放到 "
            "models/ultralytics/ 或其子目录（也可放在 ultralytics/bbox/）。"
        )
    try:
